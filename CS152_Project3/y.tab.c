...
{
       0,    55,    55,    66,    73,    95,   109,   120,   127,   181,
     215,   224,   235,   244,   250,   256,   262,   268,   274,   280,
     286,   293,   300,   306,   330,   348,   355,   362,   379,   396,
     409,   422,   428,   441,   454,   460,   473,   483,   498,   504,
     511,   526,   532,   539,   545,   553,   562,   569,   576,   584,
     591,   598,   605,   612,   619,   628,   644,   649,   655,   662,
     677,   682,   687,   693,   700,   706,   714,   725,   732,   740,
     748,   755,   764,   771,   802,   809,   818
};
#endif
...
static const char *const yytname[] =
{
  ...
  "NEQ", "NOT", "AND", "OR", "ASSIGN", "$accept", "prog", "func", "ident",
...
static const yytype_int8 yypact[] =
{
     ...
     -92,   -92,    44,    69,    73,    53,   -92,    82,   -92,    57,
       6,    81,    26,    26,    75,   -92,    63,    63,   -27,    72,
      71,    85,   -92,   -92,   -92,   -92,   -92,   -92,   -92,   -92,
      49,   -92,   -92,   -92,    26,     4,   -20,    61,    88,    55,
      58,   -92,    68,   -28,   -12,   -92,   -92,   -92,    86,     6,
      79,    87,   -27,   -92,   -27,   -92,     6,   -92,   -27,    89,
      38,   -27,   -92,   -92,     6,    26,   -92,    26,   -92,   -92,
     -92,   -92,   -92,   -92,   -92,   -27,   -27,   -27,   -92,   -27,
     -27,   -27,   -92,     6,    96,    63,   -92,    63,   -92,    90,
      91,   -92,   -92,   -92,   -92,    93,    92,   103,    55,    58,
     -92,   -28,   -28,   -12,   -12,   -12,    98,   109,    79,    87,
...
static const yytype_uint8 yytable[] =
{
     ...
     114,    18,    19,    22,    50,    57,   128,    25,   129,    26,
      27,    89,    90,    91,    92,    93,    94,    52,    53,    28,
      30,    31,    54,    51,    77,   115,    69,    28,    55,    76,
      39,    78,    84,    56,    50,    74,    85,   103,    87,   105,
     149,    89,    90,    91,    92,    93,    94,   107,   127,   133,
     142,   113,   114,   131,   132,   130,   143,   148,     7,   144,
...
static const yytype_int16 yycheck[] =
{
     ...
     121,   122,    36,    37,    96,    97,    37,    28,    42,   133,
      32,    29,     6,    11,   103,    49,   105,    33,   107,    10,
       7,    43,    44,    45,    46,    47,    48,    26,    27,    36,
       8,    34,    31,    12,     9,   131,    21,    36,    37,    28,
      37,    52,    14,    42,   133,    33,    51,    21,    50,    30,
     143,    43,    44,    45,    46,    47,    48,    30,    22,    16,
      22,    32,    32,    30,    32,    34,    17,    15,     3,   128,
...
static const yytype_uint8 yystos[] =
{
      ...
      88,    12,    26,    27,    31,    37,    42,    49,    74,    76,
      78,    79,    81,    83,    85,    86,    88,    90,    74,    21,
      88,    88,    31,    81,    33,    89,    28,     9,    52,    74,
      81,    31,    86,    79,    14,    51,    75,    50,    77,    43,
...
#line 56 "mini_l.y" /* yacc.c:1646  */
    {	
			//printf("prog");
			(yyval.patek).code = (yyvsp[-1].Terminal).code;
			*((yyval.patek).code) << (yyvsp[0].patek).code->str();
			if(!pokerchips){
				yyerror("Error. no main.");
			}
			igota21 = (yyval.patek).code;
		}
...
#line 66 "mini_l.y" /* yacc.c:1646  */
    {	
			//printf("prog2mjgmjyj");
			(yyval.patek).code = new stringstream();
			//printf("temr");
		}
			...
			*((yyval.Terminal).code) << "func " << place_holder << "\n" << (yyvsp[-7].Terminal).code->str() << (yyvsp[-4].Terminal).code->str();
			for(int i = 0; i < (yyvsp[-7].Terminal).vars->size(); ++i){
				if((*(yyvsp[-7].Terminal).vars)[i].type == INT_ARR){
					...
					*((yyval.Terminal).code) << "= " << *((*(yyvsp[-7].Terminal).vars)[i].name) << ", " << "$"<< itos(i) << "\n";
				} else {
			...
			if((yyvsp[-2].Terminal).type == INT && (yyvsp[0].Terminal).type == INT){
               *((yyval.Terminal).code) << "= " << *(yyvsp[-2].Terminal).name << ", " << *(yyvsp[0].Terminal).name << "\n";
                ...
                *((yyval.Terminal).code) << syn_create((yyvsp[-2].Terminal).value, (yyvsp[-2].Terminal).index, (yyvsp[0].Terminal).name, "[]=");
                ...
                string *tmp = temp();
                *((yyval.Terminal).code) << dot(tmp) << syn_create(tmp, (yyvsp[0].Terminal).name, (yyvsp[0].Terminal).index, "=[]");
...
#line 484 "mini_l.y" /* yacc.c:1646  */
    {
				...
				*((yyval.Terminal).code) << (yyvsp[0].Terminal).code->str();
				if((yyvsp[0].Terminal).name != NULL && (yyvsp[0].Terminal).operator1 != NULL){
					(yyval.Terminal).name = temp();
					*((yyval.Terminal).code) << dot((yyval.Terminal).name) << syn_create((yyval.Terminal).name, (yyvsp[-1].Terminal).name, (yyvsp[0].Terminal).name, *(yyvsp[0].Terminal).operator1); 
				} else {
...
#line 546 "mini_l.y" /* yacc.c:1646  */
    {
				(yyval.Terminal).code = (yyvsp[0].Terminal).code;
				(yyval.Terminal).name = temp();
				*((yyval.Terminal).code) << dot((yyval.Terminal).name) << syn_create((yyval.Terminal).name, (yyvsp[0].Terminal).name, NULL, "!");
			}
...
#line 554 "mini_l.y" /* yacc.c:1646  */
    {
				...
				*((yyval.Terminal).code) << (yyvsp[0].Terminal).code->str();
				(yyval.Terminal).name = temp();
				*((yyval.Terminal).code) << dot((yyval.Terminal).name) << syn_create((yyval.Terminal).name, (yyvsp[-2].Terminal).name, (yyvsp[0].Terminal).name, *(yyvsp[-1].Terminal).operator1);
			}
...
#line 707 "mini_l.y" /* yacc.c:1646  */
    {
			...
			(yyval.Terminal).name = temp();
			string temp = "-1";
			*((yyval.Terminal).code)<< dot((yyval.Terminal).name) << syn_create((yyval.Terminal).name, (yyvsp[0].Terminal).name, &temp, "*");
		}
...
#line 715 "mini_l.y" /* yacc.c:1646  */
    {
			...
			(yyval.Terminal).code = (yyvsp[-1].Terminal).code;
			(yyval.Terminal).name = temp();
			*((yyval.Terminal).code) << dot((yyval.Terminal).name) << "call " << (yyvsp[-3].cval) << ", " << *(yyval.Terminal).name << "\n";
...
#line 772 "mini_l.y" /* yacc.c:1646  */
    {
		...
		(yyval.Terminal).code = (yyvsp[0].Terminal).code;
		(yyval.Terminal).type = (yyvsp[0].Terminal).type;
		string tonystark = (yyvsp[-1].cval);
		dne(tonystark);
		if(map_find(tonystark) && vmap[tonystark].type != (yyvsp[0].Terminal).type){
			if((yyvsp[0].Terminal).type == INT_ARR){
		...
		if ((yyvsp[0].Terminal).index == NULL){
			...
			*((yyval.Terminal).code) << dot((yyval.Terminal).name) << syn_create((yyval.Terminal).name, pepperpots, (yyvsp[0].Terminal).index, "=[]");
...
#line 819 "mini_l.y" /* yacc.c:1646  */
    {
			...
			(yyval.Terminal).code = new stringstream();
			(yyval.Terminal).name = new string();
			*(yyval.Terminal).name = itos((yyvsp[0].val));
